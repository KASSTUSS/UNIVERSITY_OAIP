...
int main(void) 
{
    ...
    int *matrix,                 // двумерный массив (матрица)
        *checkingMatrix;         // массиив кол-ва одинковых элементов серии каждой строки
    ...
    if( !(checkingMatrix = (int*)calloc((linesMatrix), sizeof(int))) ) {
        printf("\nНедостаточно памяти.");
    ...
    for( i = 0; i < linesMatrix; i++ ) {
        for( j = 0; j < columnsMatrix; j++ ) {
            ...
            scanf("%d", (matrix + columnsMatrix*i + j));
        }
        printf("\n");
    ...
    for( i = 0; i < linesMatrix; i++ ) { // поиск строки матрицы, в которой находится самая длинная серия одинаковых элементов
        *(checkingMatrix+i) = 0;
        counterElements = 0;
        reserveElement = *(matrix + columnsMatrix*i);
        for( j = 0; j < columnsMatrix; j++ ) {
            if( *(matrix + columnsMatrix*i + j) == reserveElement ) {  
                *(checkingMatrix+i) += 1;
                counterElements++;
            } else {
                reserveElement = *(matrix + columnsMatrix*i + j);
            }
        }
    ...
    for( i = 0; i < linesMatrix; i++ ) {
        for( j = 0; j < columnsMatrix; j++ ) 
            printf("\t%d", *(matrix + columnsMatrix*i + j));
        printf("\n\n");
    }
